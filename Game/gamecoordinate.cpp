...
bool gameCoordinate::listAddSeies(QPoint block)
{
    if(isPointAvailable(block) && isPointMovable(block))
    {
        if(!showSphere.contains(block))
            showSphere.append(block);
        return true;
    }
    return false;
}
...
void gameCoordinate::listMoveSphere(QPoint block, int sphere)
{
    ...
    pair = goUpLeft(block);
    if(listAddSeies(pair))
    {
        listMoveSphere(pair, sphere);
    }

    pair = goUpRight(block);
    if(listAddSeies(pair))
    {
        listMoveSphere(pair, sphere);
    }
    ...
    pair = goDownLeft(block);
    if(listAddSeies(pair))
    {
        listMoveSphere(pair, sphere);
    }

    pair = goDownRight(block);
    if(listAddSeies(pair))
    {
        listMoveSphere(pair, sphere);
    }
